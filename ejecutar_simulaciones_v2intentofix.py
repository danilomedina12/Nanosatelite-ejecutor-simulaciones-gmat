...
Uso:
    python3 ejecutar_simulaciones_v2.py

Requiere:
    - GmatConsole instalado (modo headless)
    - SAT1_Template_v2.script en CARPETA_SCRIPTS
...
TEMPLATE        = os.path.join(CARPETA_SCRIPTS, "SAT1_Template_v2.script")
...
def extraer_metricas(nombre_escenario, delta_eclipse=None):
    """
    Extrae metricas del CSV de telemetria y del archivo de eclipses.

    BateriaWh minima: del CSV (confiable, refleja la fisica real).
    DoD max: calculado desde el archivo de eclipses + fisica.
      El ReportFile de GMAT escribe en micro-pasos del RK89 (~83s promedio),
      no en los pasos del loop de 60s. Por eso la serie de BateriaWh
      en el CSV no refleja los 34 pasos reales de cada eclipse.
      La solucion es: DoD = duracion_eclipse_s / 60 * |DeltaE_Eclipse|
    Colapso: del CSV, detectando cuando BateriaWh <= BATERIA_CRITICA.
    """
    csv_nombre = nombre_escenario + "_Telemetria.csv"
    ecl_nombre = nombre_escenario + "_Eclipses.txt"
    csv_ruta = os.path.join(CARPETA_RESULTS, csv_nombre)
    ecl_ruta = os.path.join(CARPETA_RESULTS, ecl_nombre)
    ...
    # --- Leer CSV para bateria_min y colapso ---
    if os.path.isfile(csv_ruta):
        ...
        try:
            ...
            for linea in lineas[1:]:
                ...
                # Solo contar elapsed cuando hay cambio real de bateria
                # (cada cambio = 1 paso de 60s del loop de GMAT)
                if prev_bat is None or abs(bateria_wh - prev_bat) > 1e-8:
                    elapsed += PASO_SECS
                    prev_bat = bateria_wh
    ...
    # --- Calcular DoD desde archivo de eclipses + fisica ---
    # DoD = (duracion_umbra_s / 60) * |DeltaE_Eclipse|
    # Esto es exacto porque el loop de GMAT da exactamente 1 paso
    # de 60s por cada minuto de eclipse, independiente del RK89.
    if delta_eclipse is not None and os.path.isfile(ecl_ruta):
        try:
            duraciones_umbra = []
            with open(ecl_ruta, 'r', encoding='utf-8') as f:
                for linea in f:
                    if 'Umbra' in linea and 'Earth' in linea:
                        partes = linea.split()
                        # Formato: Start Stop Duration(s) Body Type EventNum TotalDur
                        # La duracion esta en la columna 7 (index 6) despues del timestamp
                        # Timestamp ocupa 5 tokens: "22 Apr 2026 15:03:48.782"
                        # Linea: DD Mon YYYY HH:MM:SS.sss  DD Mon YYYY HH:MM:SS.sss  DUR  Earth  Umbra  N  TOT
                        # Buscar el primer numero flotante despues de los timestamps
                        numeros = []
                        for p in partes:
                            try:
                                v = float(p)
                                ...
                                pass
                        # El primer numero flotante grande es la duracion en segundos
                        # Los timestamps tienen formato HH:MM:SS.sss que no son float puros
                        durs = [v for v in numeros if 100 < v < 4000]
                        if durs:
                            duraciones_umbra.append(durs[0])

            if duraciones_umbra:
                dur_max = max(duraciones_umbra)
                pasos_eclipse = int(dur_max / PASO_SECS)
                dod_max = round(pasos_eclipse * abs(delta_eclipse), 5)
                metricas["dod_max"] = dod_max
            else:
                metricas["dod_max"] = 0.0

        except Exception as e:
            print(f"    [WARN] Error leyendo eclipses: {e}")
            metricas["dod_max"] = 0.0
    elif delta_eclipse is not None:
        # Sin archivo de eclipses: estimar con duracion tipica LEO 600km
        pasos_eclipse = int(2084 / PASO_SECS)
        metricas["dod_max"] = round(pasos_eclipse * abs(delta_eclipse), 5)
...
def main():
    ...
    for i, escenario in enumerate(ESCENARIOS, 1):
        ...
        if exito:
            print(f" OK ({dur:.0f}s)")
            metricas = extraer_metricas(nombre, delta_eclipse)
