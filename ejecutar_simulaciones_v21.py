...
CARPETA_RESULTS = "/home/danilo/Documentos/MisMisionesGMAT/resultadosNUEVO"
...
def extraer_metricas(nombre_escenario):
    ...
    try:
        ...
        # Paso 1: leer BateriaWh y detectar colapso
        # El RK89 escribe en cada micro-paso, generando miles de filas
        # con valores repetidos. Deduplicamos para obtener la serie real.
        serie_bat_raw = []
        for linea in lineas[1:]:
            ...
            serie_bat_raw.append(bateria_wh)

        # Paso 2: deduplicar la serie (eliminar micro-pasos repetidos del RK89)
        # Solo queda un valor por cada cambio real de bateria
        serie_bat = [serie_bat_raw[0]] if serie_bat_raw else []
        for v in serie_bat_raw[1:]:
            if abs(v - serie_bat[-1]) > 1e-8:
                serie_bat.append(v)

        # Paso 3: detectar DoD por ciclo usando maquina de estados picos/valles
        # DoD = maximo_local (fin de sol) - minimo_local (fin de eclipse)
        dods = []
        n = len(serie_bat)
        if n > 2:
            estado    = "subiendo"
            ultimo_max = serie_bat[0]
            ultimo_min = serie_bat[0]

            for v in serie_bat[1:]:
                if estado == "subiendo":
                    if v > ultimo_max:
                        ultimo_max = v
                    elif v < ultimo_max - 0.001:
                        estado     = "bajando"
                        ultimo_min = v
                elif estado == "bajando":
                    if v < ultimo_min:
                        ultimo_min = v
                    elif v > ultimo_min + 0.001:
                        dod = round(ultimo_max - ultimo_min, 5)
                        if dod > 0.1:   # ignorar micro-variaciones
                            dods.append(dod)
                        estado     = "subiendo"
                        ultimo_max = v
        ...
        metricas["dod_max"]     = round(max(dods), 5) if dods else 0.0
